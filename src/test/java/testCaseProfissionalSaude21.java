...
public class testCaseProfissionalSaude21 extends junit.framework.TestCase{
    ...
    public void testTestCaseProfissionalSaude21() throws Exception {
        ...
        for (int i = 0; i<linhas2.size();i++) {
            if(linhas2.get(i).getText().contains("TESTE14@mail.com")) {
        ...
        for (int i = 0; i<colunas.size();i++) {
            if(colunas.get(i).getText().contains("ESTG")) {
                colunaPretendida = linhas2.get(i);
                break;
            }
        }


        System.out.println(colunaPretendida);

        // Warning: verifyTextPresent may require manual changes
        try {
            //assertTrue(driver.findElement(By.cssSelector("BODY")).getText().contains("Rúben Emanuel Gonçalves Radamanto"));
            assertTrue("Dado que iniciei conta com prof. de saude e estou na página da Lista de Adolescentes quando clico no botão 'Editar' e altero as informações no Formulário de Edição de Utilizador e submeto o botão 'Confirmar' verifico que as alterações foram feitas",colunaPretendida.getText().contains("ESTG"));
        } catch (Error e) {
